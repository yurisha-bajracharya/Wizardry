...
void UpdatePlayer()
{
    ...
    if (IsKeyDown(KEY_UP) && !grid[player.r * ncols + player.c].walls[0])
    { // Up
        player.r--;
    }
    if (IsKeyDown(KEY_RIGHT) && !grid[player.r * ncols + player.c].walls[1])
    { // Right
        player.c++;
    }
    if (IsKeyDown(KEY_DOWN) && !grid[player.r * ncols + player.c].walls[2])
    { // Down
        player.r++;
    }
    if (IsKeyDown(KEY_LEFT) && !grid[player.r * ncols + player.c].walls[3])
    { // Left
        player.c--;
    }
    // Check for collision with Hermione
    if (player.r == hermione.r && player.c == hermione.c)
    {                   // Update this line to check against Hermione's position
